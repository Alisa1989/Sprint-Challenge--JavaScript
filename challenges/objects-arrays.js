...
let dinosaur1 = { name: 'tyrannosaurus', diet: 'carnivorous', weight: '7000kg', length: '12m', period: 'Late Cretaceous'};

// stegosaurus, herbivorous, 2000kg, 9m, Late Jurassic
let dinosaur2 = { name: 'stegosaurus', diet: 'herbivorous', weight: '2000kg', length: '9m', period: 'Late Jurassic'};

// velociraptor, carnivorous, 15kg, 1.8m, Late Cretaceous
let dinosaur3 = { name: 'velociraptor', diet: 'carnivorous', weight: '15kg', length: '1.8m', period: 'Late Cretaceous'};
...
let unisWithUni = [];
//unisWithUni = universities.filter(element => element.university.includes("Uni"));
...
a new array of strings named lowCaseAnimalNames, each string following this pattern: "jackal, asiatic". 
Log the result.
*/

let lowCaseAnimalNames = [];
lowCaseAnimalNames = zooAnimals.map( element => element.animal_name.toLowerCase());
...
all the zoos using the .reduce() method. Remember the reduce method takes two arguments: 
a callback (which itself takes two args), and an initial value for the count.

*/
let populationTotal = [];
populationTotal = zooAnimals.reduce((accum, arrItem) => accum += arrItem.population);
